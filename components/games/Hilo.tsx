...
export function Hilo(){
 ...
 const start=()=>{const b=Math.floor(Number(bet));if(!Number.isFinite(b)||b<10){setMsg('Mise minimum : 10 coins.');return}if(!spendCoins(b)){setMsg('Coins insuffisants.');return}beginGame();setCard(1+Math.floor(Math.random()*13));setActive(true);setMsg('Carte tirée !')};
 const pick=(up:boolean)=>{if(!active)return;const b=Math.floor(Number(bet)),next=1+Math.floor(Math.random()*13);setCard(next);
  // House edge: 15% of otherwise-winning calls are converted to a loss.
  // This keeps Hi-Lo from feeling like it wins too often while staying random.
  const naturalWin=up?next>card:next<card;
  const houseLoss=Math.random()<0.15;
  const win=naturalWin&&!houseLoss;
