...
const drawDifferent=(current:number)=>{const n=1+Math.floor(Math.random()*12);return n>=current?n+1:n};
export function Hilo(){
 ...
 const pick=(up:boolean)=>{if(!active)return;const b=Math.floor(Number(bet)),next=drawDifferent(card);setCard(next);const win=up?next>card:next<card;
  if(win){const p=Math.floor(b*1.8);addCoins(p);recordGameResult('Hi-Lo','win',p-b);setMsg(`🃏 ${next} — gagné +${p} coins`)}else{recordGameResult('Hi-Lo','loss',-b);setMsg(`🃏 ${next} — perdu`)}setActive(false);endGame();
 };
