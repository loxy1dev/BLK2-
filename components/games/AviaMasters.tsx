...
export function AviaMasters(){
 ...
 const start=()=>{if(running)return;const b=Math.floor(Number(bet));if(!Number.isFinite(b)||b<10){setMessage('Mise minimum : 10 coins.');return}if(!spendCoins(b)){setMessage('Coins insuffisants.');return}setBalance(getCoins());currentRef.current=b;setRoundBalance(b);setProgress(0);setAltitude(20);setEvents([]);eventRef.current=[];setLastWin(null);setRunning(true);setMessage('✈️ En vol — collecte les bonus et évite les roquettes.');let t=0;timerRef.current=setInterval(()=>{t+=0.05*speedRef.current;const p=Math.min(100,t/8.5*100);let current=currentRef.current;let alt=20+Math.sin(t*1.15)*8+t*2.1;const nextEvent=eventRef.current.length<7&&Math.random()<0.08;if(nextEvent){const rocket=Math.random()<0.24;const value=rocket?0:(Math.random()<0.72?bonuses[Math.floor(Math.random()*bonuses.length)]:multipliers[Math.floor(Math.random()*multipliers.length)]);const e={id:Date.now()+Math.random(),kind:rocket?'rocket':'bonus',value,x:18+Math.random()*70,y:18+Math.random()*58};eventRef.current=[...eventRef.current,e];setEvents([...eventRef.current]);if(rocket){current=Math.max(0,Math.round(current/2));alt-=9;setMessage('🚀 ROQUETTE ! Le compteur est divisé par 2.')}else if(value<=10){current+=value;alt+=value*1.2;setMessage(`✨ Bonus +${value} coins`)}else{current=Math.round(current*value);alt+=value*1.4;setMessage(`🔥 Multiplicateur ×${value} !`)}currentRef.current=current;setRoundBalance(current)}setProgress(p);setAltitude(Math.max(8,Math.min(88,alt)));if(p>=100)finish(Math.random()>0.15,currentRef.current)},50)};
 const toggleAuto=()=>setAutoplay(v=>!v);
 return <div className="game-panel" style={{overflow:'hidden'}}>
  <div style={{display:'flex',justifyContent:'space-between',gap:16,alignItems:'center',flexWrap:'wrap'}}><div><div className="game-kicker">ARCADE / FLIGHT</div><h2>✈️ Avia Masters</h2><p className="muted">Un jeu de vol inspiré des mécaniques de BGaming : bonus, multiplicateurs, roquettes et atterrissage.</p></div><div style={{fontSize:28,fontWeight:900}}>🪙 {coins.toLocaleString('fr-BE')}</div></div>
