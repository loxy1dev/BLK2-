...
export function TrainCash(){
 ...
 useEffect(()=>{setCoins(getCoins());return()=>{if(timer.current)clearInterval(timer.current)}},[]);
 const mult=levels[Math.min(step,levels.length-1)]||levels.at(-1)!;
 const progress=useMemo(()=>Math.min(100,step/(levels.length-1)*100),[step]);
 const start=()=>{if(running||bet<=0)return;if(!spendCoins(bet)){setMsg('Pas assez de coins.');return}beginGame('traincash');setCoins(getCoins());setStep(0);setCrashed(false);setCashed(false);setMsg('Le train part… cash out avant le crash !');setTrainX(8);setRunning(true)};
 const crash=()=>{setRunning(false);setCrashed(true);endGame();recordGameResult('traincash',0);setMsg(`💥 Crash à ×${mult.toFixed(2)} — -${bet.toLocaleString('fr-FR')} coins`);setTimeout(()=>{setCrashed(false);setStep(0)},1800)};
 const advance=()=>{if(!running)return;const next=step+1;const danger=Math.min(.07+next*.008,.23);if(Math.random()<danger){crash();return}setStep(next);setTrainX(8+Math.min(84,next/levels.length*76));setMsg(`🚂 ×${(levels[Math.min(next,levels.length-1)]).toFixed(2)} — le train continue !`)};
 const cashout=()=>{if(!running||step<1)return;const payout=Math.round(bet*mult);addCoins(payout);setCoins(getCoins());setRunning(false);setCashed(true);endGame();recordGameResult('traincash',payout);setMsg(`💰 Cash out ×${mult.toFixed(2)} → +${(payout-bet).toLocaleString('fr-FR')} coins`);setTimeout(()=>setCashed(false),1800)};
 return <div className="game-panel" style={{overflow:'hidden'}}>
  ...
  <div style={{display:'grid',gap:16}}>
   ...
   <div style={{position:'relative',height:300,borderRadius:20,overflow:'hidden',background:'linear-gradient(#17243b 0 55%,#27333a 55% 100%)',border:'1px solid rgba(255,255,255,.12)'}}>
    <div style={{position:'absolute',inset:'12% 0 auto',height:1,background:'rgba(255,255,255,.12)'}}/>
   ...
   <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:6}}>{levels.slice(0,10).map((m,i)=><div key={m} style={{padding:'8px 4px',textAlign:'center',borderRadius:9,background:i===step?'rgba(110,231,255,.2)':'rgba(255,255,255,.04)',fontSize:12,fontWeight:800}}>×{m}</div>)}</div>
