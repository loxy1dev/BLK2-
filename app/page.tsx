...
export default function Home(){const [game,setGame]=useState('towers');return <main className="container"><section className="hero premium-hero"><div><span className="eyebrow">BLOX ARCADE • NEXT GEN</span><h1>Play. Risk. <span>Win.</span></h1><p>V3 du site — entièrement fait par BLK.</p><p>Une arcade virtuelle communautaire avec progression XP, récompenses, classement mondial, chat LIVE et jeux instantanés.</p><div className="hero-pills"><span>⚡ LIVE</span><span>🏆 XP & NIVEAUX</span><span>💬 CHAT</span><span>🎁 DAILY</span></div></div><div className="hero-orb"><span>✦</span><small>PLAY<br/>NOW</small></div></section><div className="game-grid">{games.map(g=><button key={g.id} onClick={()=>setGame(g.id)} className={`game-card ${game===g.id?'active':''}`}><div className="game-icon">{g.icon}</div><div><h3>{g.name}</h3><p>{g.desc}</p></div><span>›</span></button>)}</div><div className="arcade-main-layout"><div className="arcade-main-column">{game==='shop'?<Shop/>:<section className="play-area">{game==='towers'&&<Towers/>}{game==='horses'&&<HorseRacing/>}{game==='plinko'&&<Plinko/>}{game==='blackjack'&&<Blackjack/>}{game==='roulette'&&<Roulette/>}{game==='rocket'&&<Rocket/>}{game==='dice'&&<Dice/>}{game==='hilo'&&<Hilo/>}</section>}<LiveHistory/></div><PublicChat/></div><div className="notice">🪙 Les coins sont virtuels et n'ont aucune valeur monétaire.</div></main>}
