...
async function ensureRound(now:number){const {index,startAt}=roundFor(now);const key=roundKey(index);let round=await redis.get(key) as Round|null;if(!round){const prev=index>0?await redis.get(roundKey(index-1)) as Round|null:null;const horses=makeHorses();const candidate:Round={id:`horses-${index}`,index,startAt,bettingEndsAt:startAt+BETTING_MS,endsAt:startAt+ROUND_MS,horses,winnerId:pickWinner(horses,prev?.winnerId)};const created=await redis.set(key,candidate,{nx:true,ex:120});round=created?candidate:await redis.get(key) as Round|null;}if(!round)throw new Error('ROUND_CREATE_FAILED');return round;}
...
export async function POST(req:Request){try{const s=await getSession(req);if(!s)return NextResponse.json({error:'LOGIN_REQUIRED'},{status:401});const body=await req.json().catch(()=>({}));const action=String(body?.action||'bet');const now=Date.now();const round=await ensureRound(now);const uid=s.user.id;const username=s.user.username;
 if(action==='bet'){
  ...
  const horseId=String(body?.horseId||'');const horse=round.horses.find(h=>h.id===horseId);const bet=validBet(body?.bet);
  ...
  const key=betKey(round.id,uid);const current=Number(await redis.hget(key,horse.id)||0);if(current+bet>MAX_BET)return NextResponse.json({error:`Maximum ${MAX_BET.toLocaleString('fr-BE')} coins sur ce cheval.`},{status:400});
 ...
 if(action==='claim'){
  const targetIndex=Number(body?.roundIndex??round.index);const targetId=String(body?.roundId||'');if(!Number.isSafeInteger(targetIndex)||Math.abs(targetIndex-round.index)>1)return NextResponse.json({error:'Course introuvable.'},{status:404});
  ...
  const bets=await readBets(target.id,uid);const totalBet=Object.values(bets).reduce((a,b)=>a+b,0);if(totalBet<=0){const empty={payout:0,net:0,bet:0,winnerId:target.winnerId,odds:0,horseName:''};await redis.set(claimKey(target.id,uid),empty,{ex:120});return NextResponse.json({...empty,roundId:target.id,roundIndex:target.index});}
  const winner=target.horses.find(h=>h.id===target.winnerId)!;const winningBet=Number(bets[target.winnerId]||0);const payout=winningBet>0?Math.floor(winningBet*winner.odds):0;const net=payout-totalBet;const type:payout extends never?'win':'win'='win';
  await addResult(uid,username,s.user.createdAt,'Horses',payout>0?'win':'loss',net);await addHistory({roundId:target.id,username,horse:winner.name,type:payout>0?'win':'loss',amount:net,payout,bet:totalBet,odds:winner.odds,createdAt:Date.now()});
